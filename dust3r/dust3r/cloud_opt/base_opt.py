...
from dust3r.cloud_opt.commons import (edge_str, ALL_DISTS, NoGradParamDict,
                                      get_imshapes, signed_expm1, signed_log1p,
...
class BasePCOptimizer(nn.Module):
    ...
    def _init_from_views(self,
        ...
        self.pw_poses = nn.Parameter(
            rand_pose((self.n_edges, 1 + self.POSE_DIM)))  # pairwise poses
        self.pw_adaptors = nn.Parameter(torch.zeros(
            (self.n_edges, 2)))  # slight xy/z adaptation
    ...
    def state_dict(self, trainable=True):
        ...
        return {
            ...
            if k.startswith(('_', 'pred_i.', 'pred_j.', 'conf_i.',
                             'conf_j.')) != trainable
        }
    ...
    def get_adaptors(self):
        ...
        adapt = torch.cat((adapt[:, 0:1], adapt),
                          dim=-1)  # (scale_xy, scale_xy, scale_z)
    ...
    def _set_pose(self, poses, idx, R, T=None, scale=None, force=False):
        ...
        if T is not None:
            pose.data[4:7] = signed_log1p(
                T / (scale or 1))  # translation is function of scale
    ...
    def get_pw_poses(self):  # cam to world
        ...
        scaled_RT[:, :3] *= self.get_pw_scale().view(
            -1, 1, 1)  # scale the rotation AND translation
        return scaled_RT
    ...
    def forward(self, ret_details=False):
        ...
        for e, (i, j) in enumerate(self.edges):
            ...
            li = self.dist(proj_pts3d[i], aligned_pred_i,
                           weight=weight_i[i_j]).mean()
            lj = self.dist(proj_pts3d[j], aligned_pred_j,
                           weight=weight_j[i_j]).mean()
            loss = loss + li + lj
    ...
    def compute_global_alignment(self, init=None, niter_PnP=10, **kw):
        """
### 목적
위 코드는 DUST3R 모델의 글로벌 정렬 최적화를 수행하는 함수들로 구성되어 있음
이 최적화는 여러 이미지로부터 추출된 포인트 클라우드와 카메라 포즈를 이용하여
    전체 장면의 일관된 3D 재구성을 목표로 합니다.
...
3. **인자**:
   ...
   - `kw`: 추가적인 키워드 인자들.
        ...
        if init is None:
            ...
            init_fun.init_from_known_poses(self,
    ...
    def show(self,
        ...
        if self.imgs is None:
            ...
            for n in range(self.n_imgs):
                viz.add_pointcloud(self.get_pts3d()[n], colors[n],
                                   self.get_masks()[n])
        else:
...
def global_alignment_loop(net,
    ...
    if verbose:
        ...
        print([
            name for name, value in net.named_parameters()
    ...
    if verbose:
        with tqdm.tqdm(total=niter) as bar:
            while bar.n < bar.total:
                loss, lr = global_alignment_iter(net, bar.n, niter, lr_base,
                                                 lr_min, optimizer, schedule)
                bar.set_postfix_str(f'{lr=:g} loss={loss:g}')
                bar.update()
    else:
        for n in range(niter):
            loss, _ = global_alignment_iter(net, n, niter, lr_base, lr_min,
                                            optimizer, schedule)
    return loss
...
def clean_pointcloud(im_confs,
    ...
    """
    assert len(im_confs) == len(cams) == len(K) == len(depthmaps) == len(
        all_pts3d)
    assert 0 <= tol < 1
    ...
    for i, pts3d in enumerate(all_pts3d):
        for j in range(len(all_pts3d)):
            ...
            bad_points = (proj_depth[msk_i] < (1 - tol) *
                          depthmaps[j][msk_j]) & (res[i][msk_i] < res[j][msk_j])
