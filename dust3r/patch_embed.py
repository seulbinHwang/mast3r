...
class ManyAR_PatchEmbed(PatchEmbed):
    ...
    def forward(self, img, true_shape):
        ...
        assert true_shape.shape == (
            B, 2), f"true_shape has the wrong shape={true_shape.shape}"
        ...
        x[is_landscape] = self.proj(img[is_landscape]).permute(
            0, 2, 3, 1).flatten(1, 2).float()
        x[is_portrait] = self.proj(img[is_portrait].swapaxes(-1, -2)).permute(
            0, 2, 3, 1).flatten(1, 2).float()
