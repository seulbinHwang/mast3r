...
def init_from_known_poses(self, niter_PnP=10, min_conf_thr=3):
    ...
    for e, (i, j) in enumerate(tqdm(self.edges, disable=not self.verbose)):
        ...
        s, R, T = align_multiple_poses(torch.stack((P1, P2)),
                                       known_poses[[i, j]])
...
def init_from_pts3d(self, pts3d, im_focals, im_poses):
    ...
    if nkp == 1:
        ...
        s, R, T = align_multiple_poses(im_poses[known_poses_msk],
                                       known_poses[known_poses_msk])
    ...
    for e, (i, j) in enumerate(self.edges):
        ...
        s, R, T = rigid_points_registration(self.pred_i[i_j],
                                            pts3d[i],
                                            conf=self.conf_i[i_j])
        self._set_pose(self.pw_poses, e, R, T, scale=s)
...
def minimum_spanning_tree(imshapes,
    ...
    while todo:
        ...
        if i in done:
            ...
            i_j = edge_str(i, j)
            s, R, T = rigid_points_registration(pred_i[i_j],
                                                pts3d[i],
                                                conf=conf_i[i_j])
            ...
            i_j = edge_str(i, j)
            s, R, T = rigid_points_registration(pred_j[i_j],
                                                pts3d[j],
                                                conf=conf_j[i_j])
            trf = sRT_to_4x4(s, R, T, device)
    ...
    if has_im_poses:
        ...
        pair_scores = list(
            sparse_graph.values())  # already negative scores: less is best
        edges_from_best_to_worse = np.array(list(
            sparse_graph.keys()))[np.argsort(pair_scores)]
...
def rigid_points_registration(pts1, pts2, conf):
    R, T, s = roma.rigid_points_registration(pts1.reshape(-1, 3),
                                             pts2.reshape(-1, 3),
                                             weights=conf.ravel(),
...
def estimate_focal(pts3d_i, pp=None):
    ...
    focal = estimate_focal_knowing_depth(pts3d_i.unsqueeze(0),
                                         pp.unsqueeze(0),
                                         focal_mode='weiszfeld').ravel()
    return float(focal)
...
def fast_pnp(pts3d, focal, msk, device, pp=None, niter_PnP=10):
    ...
    for focal in tentative_focals:
        ...
        success, R, T, inliers = cv2.solvePnPRansac(pts3d[msk],
                                                    pixels[msk],
                                                    K,
                                                    None,
...
def align_multiple_poses(src_poses, target_poses):
    ...
    def center_and_z(poses):
        ...
        return torch.cat((poses[:, :3,
                                3], poses[:, :3, 3] + eps * poses[:, :3, 2]))

    R, T, s = roma.rigid_points_registration(center_and_z(src_poses),
