...
class VislocAachenDayNight(BaseVislocColmapDataset):
    ...
    def __init__(self, root, subscene, pairsfile, topk=1, cache_sfm=False):
        ...
        pairsfile_path = os.path.join(root, 'pairsfile/query',
                                      pairsfile + '.txt')
        super().__init__(image_path=image_path,
                         ...
                         cache_sfm=cache_sfm)
        self.scenes = [
            filename for filename in self.scenes if filename in self.pairs
        ]
        if self.subscene == 'day' or self.subscene == 'night':
            self.scenes = [
                filename for filename in self.scenes
                if self.subscene in filename
            ]
