...
def get_args_parser():
    ...
    parser_url.add_argument(
        ...
        default=False,
        help=
        "make app accessible on local network: address will be set to 0.0.0.0")
    parser_url.add_argument("--server_name",
                            type=str,
                            default=None,
...
def set_print_with_timestamp(time_format="%Y-%m-%d %H:%M:%S"):
    ...
    def print_with_timestamp(*args, **kwargs):
        ...
        builtin_print(f'[{formatted_date_time}] ',
...
def _convert_scene_output_to_glb(outdir,
                                 imgs,
                                 pts3d,
                                 mask,
                                 focals,
...
def get_3D_model_from_scene(outdir,
    ...
    return _convert_scene_output_to_glb(outdir,
                                        rgbimg,
                                        pts3d,
                                        msk,
                                        focals,
...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist,
    ...
    outfile = get_3D_model_from_scene(outdir, silent, scene, min_conf_thr,
...
def main_demo(tmpdirname,
              ...
              server_port,
              silent=False):
    recon_fun = functools.partial(get_reconstructed_scene, tmpdirname, model,
                                  device, silent, image_size)
    model_from_scene_fun = functools.partial(get_3D_model_from_scene,
                                             tmpdirname, silent)
    with gradio.Blocks(
        ...
        with gradio.Column():
            ...
            with gradio.Row():
                ...
                cam_size = gradio.Slider(label="cam_size",
                                         ...
                                         step=0.001)
            with gradio.Row():
                as_pointcloud = gradio.Checkbox(value=False,
                                                label="As pointcloud")
            ...
            run_btn.click(fn=recon_fun,
                          inputs=[
                              inputfiles, schedule, niter, min_conf_thr,
                              as_pointcloud, mask_sky, clean_depth,
