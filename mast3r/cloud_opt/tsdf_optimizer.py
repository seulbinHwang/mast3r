...
class TSDFPostProcess:
    ...
    def __init__(self,
        ...
        pts3d, depthmaps, confs = optimizer.get_dense_pts3d(clean_depth=False,
                                                            subsample=subsample)
        pts3d, depthmaps = self._TSDF_postprocess_or_not(
            pts3d, depthmaps, confs)
        self.pts3d = pts3d
    ...
    def _get_depthmaps(self, TSDF_filtering_thresh=None):
        if TSDF_filtering_thresh:
            self._refine_depths_with_TSDF(
                self.optimizer,
                TSDF_filtering_thresh)  # compute refined depths if needed
        dms = self.TSDF_im_depthmaps if TSDF_filtering_thresh else self.im_depthmaps
        return [d.exp() for d in dms]

    @torch.no_grad()
    def _refine_depths_with_TSDF(self,
                                 TSDF_filtering_thresh,
                                 niter=1,
                                 nsamples=1000):
        """
        ...
        alldepths, allposes, allfocals, allpps, allimshapes = self._get_depthmaps(
        ), self.optimizer.get_im_poses(), self.optimizer.get_focals(
        ), self.optimizer.get_principal_points(), self.imshapes
        for vi in tqdm(range(self.optimizer.n_imgs)):
            dm, pose, focal, pp, imshape = alldepths[vi], allposes[
                vi], allfocals[vi], allpps[vi], allimshapes[vi]
            ...
            for it in range(niter):
                ...
                curproj = self._backproj_pts3d(
                    ...
                    in_pps=pp[None],
                    in_imshapes=[imshape])[0]  # [H,W,Nsamp,3]
                # Batched TSDF eval
                ...
                for batch in range(0, len(curproj), self.TSDF_batchsize):
                    values, valid = self._TSDF_query(
                        curproj[batch:min(batch +
                                          self.TSDF_batchsize, len(curproj))],
                        curthresh)
    ...
    def _TSDF_query(self, qpoints, TSDF_filtering_thresh, weighted=True):
        ...
        image_coords = coords_and_depth[..., :2].round().to(
            int)  # for now, there's no interpolation...
        ...
        all_TSDF_scores = all_SDF_scores.clip(-TSDF_filtering_thresh,
                                              1e20)  # SDF -> TSDF
    ...
    def _get_pixel_depths(self,
                          image_coords,
                          TSDF_filtering_thresh=None,
                          with_normals_conf=False):
        ...
        curconfs = self._get_confs_with_normals(
        ) if with_normals_conf else self.im_conf
        for ni, (imc, depth, conf) in enumerate(
                zip(image_coords, self._get_depthmaps(TSDF_filtering_thresh),
                    curconfs)):
            H, W = depth.shape
        ...
        class Sobel(nn.Module):
            ...
            def __init__(self):
                ...
                self.filter = nn.Conv2d(in_channels=1,
                                        ...
                                        padding=1,
                                        bias=False)
                Gx = torch.tensor([[2.0, 0.0, -2.0], [4.0, 0.0, -4.0],
                                   [2.0, 0.0, -2.0]])
                Gy = torch.tensor([[2.0, 4.0, 2.0], [0.0, 0.0, 0.0],
                                   [-2.0, -4.0, -2.0]])
    ...
    def _proj_pts3d(self, pts3d, cam2worlds, focals, pps):
        ...
        # prepare intrinsics
        intrinsics = torch.eye(3).to(cam2worlds)[None].repeat(
            focals.shape[0], 1, 1)
    ...
    def _backproj_pts3d(self,
        ...
        im_poses = self.optimizer.get_im_poses(
        ) if in_im_poses is None else in_im_poses
        ...
        dm_to_3d = [
            depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[[i]])
    ...
    def _depth_to_pts3d(self,
        ...
        dm_to_3d = [
            depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[i:i + 1])
            for i in range(im_poses.shape[0])
        ]
        ...
        return [
            geotrf(pp, autoprocess(pt)) for pp, pt in zip(im_poses, dm_to_3d)
        ]
    ...
    def _get_pts3d(self, TSDF_filtering_thresh=None, **kw):
        ...
        """
        return self._backproj_pts3d(in_depths=self._get_depthmaps(
            TSDF_filtering_thresh=TSDF_filtering_thresh),
                                    **kw)
    ...
    def _TSDF_postprocess_or_not(self, pts3d, depthmaps, confs, niter=1):
        ...
        self.im_depthmaps = [
            dd.log().view(imshape)
            for dd, imshape in zip(depthmaps, self.imshapes)
        ]
        self.im_conf = confs
    ...
    def get_dense_pts3d(self, clean_depth=True):
        if clean_depth:
            confs = clean_pointcloud(self.confs, self.optimizer.intrinsics,
                                     inv(self.optimizer.cam2w), self.depthmaps,
                                     self.pts3d)
