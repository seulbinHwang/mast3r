...
def batched_triangulate(
        pts2d,  # [B, Ncams, Npts, 2]
        proj_mats):  # [B, Ncams, 3, 4] I@E projection matrix
    B, Ncams, Npts, two = pts2d.shape
    assert two == 2
    assert proj_mats.shape == (B, Ncams, 3, 4)
    # P - xP
    x = proj_mats[..., 0, :][..., None, :] - torch.einsum(
        'bij,bik->bijk', pts2d[..., 0], proj_mats[...,
                                                  2, :])  # [B, Ncams, Npts, 4]
    y = proj_mats[..., 1, :][..., None, :] - torch.einsum(
        'bij,bik->bijk', pts2d[..., 1], proj_mats[...,
                                                  2, :])  # [B, Ncams, Npts, 4]
...
def matches_to_depths(
    intrinsics,  # input camera intrinsics     [B, Ncams, 3, 3]
    ...
    for b in range(B // batchsize + 1):  # batched processing
        ...
        if sub_batchsize == 0:
            continue
        points1, points2, confs = matches[sub_batch, ..., :2], matches[
            sub_batch, ..., 2:4], matches[sub_batch, ..., -1]
        allpoints = torch.cat([
            points1.view([sub_batchsize * Nv, 1, H * W, 2]),
            points2.view([sub_batchsize * Nv, 1, H * W, 2])
        ],
                              dim=1)  # [BxNv, 2, HxW, 2]

        allcam_Ps = intrinsics[sub_batch] @ extrinsics[sub_batch, :, :3, :]
        cam_Ps1, cam_Ps2 = allcam_Ps[:, [0]].repeat(
            [1, Nv, 1, 1]), allcam_Ps[:, 1:]  # [B, Nv, 3, 4]
        formatted_camPs = torch.cat([
            cam_Ps1.reshape([sub_batchsize * Nv, 1, 3, 4]),
            cam_Ps2.reshape([sub_batchsize * Nv, 1, 3, 4])
        ],
                                    dim=1)  # [BxNv, 2, 3, 4]

        # Triangulate matches to 3D
        points_3d_world = batched_triangulate(
            allpoints, formatted_camPs)  # [BxNv, HxW, three]
        ...
        assert torch.all(
            torch.logical_or(validsuni == 0, validsuni == 3)
        ), "Error, can only be nan for none or all XYZ values, not a subset"
        ...
        points_3d_world[valids] /= normalization[
            valids_sum == 3][:, None].repeat(1, 3).view(-1)
        points_3d_world[~valids] = 0.
        aggregated_points = points_3d_world.sum(
            dim=1)  # weighted average (by confidence value) ignoring nans

        # Reset invalid values to nans, with a min visibility threshold
        aggregated_points[valids_sum.sum(dim=1) /
                          3 <= min_num_valids] = torch.nan

        # From 3D to depths
        refcamE = extrinsics[sub_batch, 0]
        points_3d_camera = (refcamE[:, :3, :3] @ aggregated_points.view(
            sub_batchsize, -1, 3).transpose(-2, -1) +
                            refcamE[:, :3, [3]]).transpose(-2, -1)  # [B,HxW,3]
        depths = points_3d_camera.view(sub_batchsize, H, W, 3)[...,
                                                               2]  # [B,H,W]
