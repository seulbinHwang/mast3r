...
cat_collate_fn_map[type(
    None
)] = lambda _, **kw: None  # When some Nones, simply return a single None
...
def cat_collate(batch,
    ...
    if isinstance(elem, collections.abc.Mapping):
        try:
            return elem_type({
                key:
                    cat_collate([d[key] for d in batch],
        ...
        return elem_type(*(cat_collate(samples, collate_fn_map=collate_fn_map)
        ...
        if isinstance(elem, tuple):
            ...
            return [
                cat_collate(samples, collate_fn_map=collate_fn_map)
