...
def postprocess(out,
    ...
    if desc_dim is not None:
        ...
        res['desc'] = reg_desc(fmap[..., start:start + desc_dim],
                               mode=desc_mode)
        if two_confs:
            res['desc_conf'] = reg_dense_conf(fmap[..., start + desc_dim],
                                              mode=desc_conf_mode)
        else:
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __init__(self,
        ...
        if isinstance(patch_size, tuple):
            assert len(patch_size) == 2 and isinstance(
                patch_size[0], int
            ) and isinstance(
                patch_size[1], int
    ...
    def forward(self, decout, img_shape):
        ...
        local_features = F.pixel_shuffle(local_features,
                                         self.patch_size)  # B,d,H,W
...
def mast3r_head_factory(head_type, output_mode, net, has_conf=False):
    ...
    if head_type == 'catmlp+dpt' and output_mode.startswith('pts3d+desc'):
        ...
        return Cat_MLP_LocalFeatures_DPT_Pts3d(
            net,
