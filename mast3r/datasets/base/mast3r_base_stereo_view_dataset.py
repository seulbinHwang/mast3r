...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    ...
    def __init__(
        ...
        self.nneg = nneg
        assert self.n_corres == 'all' or isinstance(self.n_corres, int) or (
            isinstance(self.n_corres, list) and
            len(self.n_corres) == self.num_views
    ...
    def generate_crops_from_pair(self,
                                 view1,
                                 view2,
                                 resolution,
                                 aug_crop_arg,
        ...
        if aug_crop_arg is False:
            ...
            for i in range(2):
                view = views[i]
                view['img'], view['depthmap'], view[
                    'camera_intrinsics'] = self._crop_resize_if_necessary(
                        view['img'],
                        view['depthmap'],
                        ...
                        rng=rng)
                view['pts3d'], view[
                    'valid_mask'] = depthmap_to_absolute_camera_coordinates(
                        view['depthmap'], view['camera_intrinsics'],
                        view['camera_pose'])
            return

        # extract correspondences
        corres = extract_correspondences_from_pts3d(*views,
                                                    target_n_corres=None,
                                                    rng=rng)
        ...
        for i in range(2):
            view = views[i]
            imsize, K_new, R, H = crop_to_homography(view['camera_intrinsics'],
                                                     crops[i],
                                                     crops_resolution[i])
            ...
            depthmap2 = depthmap_to_camera_coordinates(view['depthmap'],
                                                       K_old)[0] @ R[:, 2]
            ...
            if 'track_labels' in view:
                ...
                mapping, track_labels = np.unique(view['track_labels'],
                                                  return_inverse=True)
    ...
    def __getitem__(self, idx):
        ...
        self.generate_crops_from_pair(views[0],
                                      views[1],
                                      resolution=resolution,
        ...
        for v, view in enumerate(views):
            if 'corres' not in view:
                view['corres'] = np.full((self.n_corres, 2),
                                         np.nan,
                                         dtype=np.float32)
...
def rotate_90(views, k=1):
    ...
    for view in views:
        view['img'] = torch.rot90(
            view['img'], k=k,
            dims=(-2, -1))  # WARNING!! dims=(-1,-2) != dims=(-2,-1)
        ...
        view['camera_intrinsics'][:2,
                                  2] = geotrf(RT2, view['camera_intrinsics'][:2,
                                                                             2])
        if k % 2 == 1:
            K = view['camera_intrinsics']
        ...
        view['corres'] = geotrf(RT2, view['corres']).round().astype(
            view['corres'].dtype)
